...
var y_final: int
var velocite: float
const vitesse_init: float = Global.JETON.VITESSE_INITIALE
@onready var vitesse: float = vitesse_init
...
func _process(delta):
	transform.origin.y += delta*vitesse
	vitesse += delta * Global.JETON.ACCELERATION
